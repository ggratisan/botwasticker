    ...
    if (cmd) {
      ...
      switch (cmd[0]) {
        ...
        case '#help':
          client.sendText(
            message.from,
            'Menu: \n1. #sticker / #stiker: Kirim dengan caption #stike atau reply gambar yang sudah dikirim. \n2. #sticker / #stiker spasi url gambar (contoh: #stiker https://avatars2.deryl.com/u/wkwk)'
          )
          break
        case '#sticker':
        case '#stiker':
          if (message.isMedia) {
            ...
            if (url.match(isUrl)) {
              ...
              client.sendText(
                message.from,
                'Maaf, urlnya tidak valid'
              )
            }
          } else {
            client.sendText(
              message.from,
              'Butuh gambat! send gambat dengan caption #stiker'
            )
          }
          break
      }
